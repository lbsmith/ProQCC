...
*/
    ...
    CPrintf("$f9%6i strofs\n", strofs);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%6i numstatements\n", numstatements);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%6i numfunctions\n", numfunctions);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%6i numglobaldefs\n", numglobaldefs);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%6i numfielddefs\n", numfielddefs);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%6i numpr_globals\n", numpr_globals);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
    ...
    CPrintf("$f9%6i TOTAL SIZE\n", (int)lseek(h, 0, SEEK_CUR));
    mainstatusline++;
    MoveCurs(3, mainstatusline);
 ...
 */
	...
	if (d->type->type == ev_function && !d->scope) {	/*
	     ...
	     */
	    if (!d->initialized) {
		Error("function %s was not defined\n", d->name);
		errors = true;
	    }
	}
    }
 ...
 */
void
PackFile(char *src, char *name)
{
    int in;
    int remaining, count;
    char buf[4096];

    if ((byte *) pf - (byte *) pfiles > (int)sizeof(pfiles))
...
CopyFiles(void)
{
    ...
    MoveCurs(50, mainstatusline);
    CPrintf("$f9%3i unique precache_sounds", numsounds);
    mainstatusline++;
    MoveCurs(50, mainstatusline);

    CPrintf("$f9%3i unique precache_models", nummodels);
    mainstatusline++;
    MoveCurs(50, mainstatusline);
...
void De_ProgsDat(char *name);
void De_ReadData(char *name);
void De_PrintFunction(char *name);
...
main(int argc, char **argv)
{
    ...
    if (CheckParm("-?") || CheckParm("-help") || CheckParm("/?")) {
	printf("ProQCC command line options:\n\n");
	printf("By default, ProQCC looks for progs.src in the current directory\n");
	printf("-src <directory>     : look in a different directory\n");
	printf("-dec [datfile]       : decompile progs.dat\n");
	printf("-copy <srcdir>       : build a clean data tree\n");
	printf("-pak <srcdir>        : build a clean pak file\n");
	printf("-bspmodels <gamedir> : bsp all bmodels\n\n");
	return;
    }
    if (CheckParm("-nolog")) {
        logging = FALSE;
...
//    CPrintf("$b4$f7ProQCC$f6 - Advanced Quake-C Compiler/Decompiler $f7CUSTOM VERSION$f6    Version $f7" VERSION);
    ...
    src = COM_Parse(src);
    if (!src)
	Error("No destination filename.  proqcc -help for info.\n");
    strcpy(destfile, com_token);

    CPrintf("Outputfile: %s\n", destfile);
    mainstatusline++;
    MoveCurs(3, mainstatusline);
	...
	sprintf(filename, "%s%s", sourcedir, com_token);
	ShowTempEntry("compiling %s", filename);
