...
Error(char *format,...)
{
    ...
    ShowWarningEntry("There were some errors ***");
...
SafeOpenWrite(char *filename)
{
    ...
    handle = open(filename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0644);
...
void 
SafeRead(int handle, void *buffer, long count)
{
...
void 
SafeWrite(int handle, void *buffer, long count)
{
 ...
 */
void 
SaveFile(char *filename, void *buffer, long count)
{
    int handle;
