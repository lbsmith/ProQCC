...
PR_Expression(int priority)
{
	...
	for (op = pr_opcodes; op->name; op++) {
		...
		op++;
		if (!op->name || DefCmp(op->name, oldop->name))
 ...
 */
def_t *
PR_GetDef(type_t * type, char *name, def_t * scope, boolean allocate)
{
#undef cacheSize
#undef cacheMask
